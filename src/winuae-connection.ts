...
export class WinUAEConnection {
  ...
  async connect(connectBehavior: WinUAEConnectBehavior = {}): Promise<void> {
    ...
    const defaultUaePath = path.join(launchCwd, 'default.uae');
    ...
    } else if (legacyLaunch) {
      trace(`Legacy launch with -f: ${configFileAbs}`);
    } else {
      // Match vscode-amiga-debug: write default.uae in cwd, launch with -portable only.
      fs.writeFileSync(defaultUaePath, fs.readFileSync(configFileAbs, 'utf8'), 'utf8');
      trace(`Extension-style launch: wrote ${defaultUaePath}`);
    }
    ...
    const args: string[] = [];
    if (legacyLaunch && useConfigFile && configFileAbs) {
